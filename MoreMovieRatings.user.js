...
function insertDoubanRatingDiv(parent, title, rating, link, num_raters) {
    var star = (5 * Math.round(rating)).toString();
    if (star.length == 1)
        star = '0' + star;
    if (typeof rating === 'number')
        rating = rating.toFixed(1);//把数字转化为字符串，参数指定小数位数
        ...
        getIMDbInfo(id, function (data) {
            ...
            if (tomato_score) {
                ...
                if (parseInt(tomato_score) >= 60)
                    ...
                    fresh = 'rotten';
                var tomato_url = data.tomatoURL.replace('http://', 'https://');
                ratings.insertAdjacentHTML('beforeend',
